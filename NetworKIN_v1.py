...
BLAST_DIR = os.path.join(BASE_DIR, "blast-2.2.17", "bin")
# --------------------------
...
def runNetPhorest(id_seq, id_pos_res, save_diskspace, number_of_processes, number_of_active_processes=1, fast=False,
    ...
    # HARDCODED: Returns your specific results file
    return [CDummy("results.txt")]
...
def parseNetphorestFile(filename, id_pos_res, save_diskspace = True):
	...
	with open(filename) as infile:
        ...
        # FIX: Handle pynetphorest output (which lacks the 'organism' column)
        # pynetphorest columns: Name(0), Pos(1), Res(2), Pep(3), Method(4), Tree(5), Classifier(6), ...
        try:
            res = tokens[2]
            peptide = tokens[3]
            method = tokens[4]
            tree = tokens[5]  # "KIN" or "SH2"
            pred = tokens[6]  # The specific kinase group
            # pynetphorest doesn't output organism column, so we ignore it
            organism = "human"
        except IndexError:
