...
import os, subprocess, re, tempfile, glob, random
...
def mapPeptides2STRING(blastDir, organism, fastafilename, id_pos_res, id_seq, number_of_processes, datadir, fast=True,
    ...
    blastDB = os.path.join(datadir, f"{organism}.protein.sequences.v9.0.fa")
...
def printResult(id_pos_tree_pred, tree_pred_string_data, incoming2string, string_alias, string_desc, organism, mode,
    ...
    # ---------------------------------------------------------
    # 1. Load Likelihood Conversion Tables (Bayesian Models)
    # ---------------------------------------------------------
    search_path = os.path.join(dir_likelihood_conversion_tbl, "conversion_tbl_*_smooth*")

    # Debug info
    sys.stderr.write(f"Loading tables from: {search_path}\n")

    for fname in glob.glob(search_path):
        try:
            basename = os.path.basename(os.path.splitext(fname)[0])
            # Regex to parse: e.g., conversion_tbl_netphorest_smooth_human_KIN_CDK2
            match = re.search(r"conversion_tbl_([a-z]+)_smooth_([a-z]+)_([A-Z0-9]+)_([a-zA-Z0-9_/-]+)", basename)
            if match:
                netphorest_or_string = match.group(1)  # 'netphorest' or 'string'
                species_tbl = match.group(2)  # 'human'
                tree = match.group(3)  # 'KIN' or 'SH2'
                player_name = match.group(4)  # 'CDK2' or 'general'

                if species_tbl != species: continue

                conversion_tbl = ReadConversionTableBin(fname)
                # Store in nested dict: [species][tree][name][type]
                SetValueIntoMultiLevelDict(dLRConvTbl, [species_tbl, tree, player_name, netphorest_or_string],
                                           conversion_tbl)
        except Exception as e:
            sys.stderr.write(f"Error loading table {fname}: {e}\n")
            continue
        ...
        if id in incoming2string:
            for pos in id_pos_tree_pred[id]:
                for tree in id_pos_tree_pred[id][pos]:
                        ...
                        for string1 in incoming2string[id]:
                            ...
                            if string1 in tree_pred_string_data:
                                ...
                                for string2 in tree_pred_string_data[string1]:
                                    ...
                                    # -------------------------------------------------
                                    # 3. Domain Mapping & Table Selection Logic
                                    # -------------------------------------------------
                                    tbl_net = []
                                    tbl_str = []

                                    # Check if the STRING protein (string2) matches the NetPhorest group (tree/pred)
                                    is_mapped = False
                                    if tree in map_group2domain and pred in map_group2domain[tree]:
                                        if name in map_group2domain[tree][pred]:
                                            is_mapped = True

                                    # A. UNCOVERED CASE (Kinase not in known mapping)
                                    if not is_mapped:
                                        if options.string_for_uncovered:
                                            # Use "general" SH2 tables for specific non-kinase domains
                                            if species == "human" and tree in ["1433", "BRCT", "WW", "PTB", "WD40",
                                                                               "FHA"]:
                                                tbl_str = dLRConvTbl[species]["SH2"]["general"]["string"]
                                            else:
                                                # Use general table for the tree (e.g., general KIN)
                                                if "general" in dLRConvTbl.get(species, {}).get(tree, {}):
                                                    tbl_str = dLRConvTbl[species][tree]["general"]["string"]
                                                else:
                                                    continue  # Skip if no general table

                                            likelihood_netphorest = 1.0
                                            likelihood_string = ConvertScore2L(stringScore, tbl_str)
                                            networkinScore = likelihood_netphorest * likelihood_string
                                        else:
                                            continue  # Skip unmatched if flag is off

                                    # B. MAPPED CASE (Standard NetworKIN logic)
                                    else:
                                        # Determine specific vs general table
                                        if species == "human":
                                            # Special handling: Group various domains under SH2 logic
                                            if tree in ["1433", "BRCT", "WW", "PTB", "WD40", "FHA"]:
                                                tbl_net = dLRConvTbl[species]["SH2"]["general"]["netphorest"]
                                                tbl_str = dLRConvTbl[species]["SH2"]["general"]["string"]
                                            else:
                                                # Check if specific kinase name exists in tables
                                                if tree in dLRConvTbl.get(species, {}) and name in dLRConvTbl[species][
                                                    tree]:
                                                    tbl_net = dLRConvTbl[species][tree][name]["netphorest"]
                                                    tbl_str = dLRConvTbl[species][tree][name]["string"]
                                                else:
                                                    # Fallback to general
                                                    tbl_net = dLRConvTbl[species][tree]["general"]["netphorest"]
                                                    tbl_str = dLRConvTbl[species][tree]["general"]["string"]
                                        ...
                                        # Calculate Unified Likelihood
                                        likelihood_netphorest = ConvertScore2L(netphorestScore, tbl_net)
                                        likelihood_string = ConvertScore2L(stringScore, tbl_str)
                                        networkinScore = likelihood_netphorest * likelihood_string
                                    ...
                                    # Note: If uncovered, netphorest score is N/A in output usually, but we keep it simple
                                    net_disp = "N/A" if not is_mapped else f(netphorestScore)

                                    result = (f"{id}\t{res}{pos}\t{tree}\t{pred}\t{name}\t"
                                              f"{f(networkinScore)}\t{net_disp}\t{f(stringScore)}\t"
                                              f"{string1}\t{string2}\t{bestName1}\t{bestName2}\t"
                                              f"{desc1}\t{desc2}\t{peptide}\t{path}\n")
                    ...
                    # -------------------------------------------------
                    # 5. Output Selection (Network vs All)
                    # -------------------------------------------------
                    if mode == 'network' and score_results:
                        # Get highest score key
                        highestScore = sorted(score_results.keys(), reverse=True)[0]
                        # If multiple candidates have the exact same highest score, pick random
                        candidates = score_results[highestScore]
                        if len(candidates) > 1:
                            sys.stdout.write(candidates[random.randint(0, len(candidates) - 1)])
                        else:
                            sys.stdout.write(candidates[0])
                    else:
                        # Print all, sorted by score
